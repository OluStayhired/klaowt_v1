...
function App() {
  ...
  const [feedType, setFeedType] = useState<'popular' | 'suggested' | 'create' | 'pinned' | 'personal'>('popular');
...
// Set up login success handler
  ...
  // Add any additional user click handling logic here
  // For example, opening a user profile modal or sidebar
};
  ...
  const handleFeedTypeChange = (type: 'popular' | 'suggested' | 'create' | 'pinned' | 'personal') => {
    setFeedType(type);
  };
        ...
        <div className="flex space-x-6 order-1 sm:order-2"> 
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
            ...
            <div className="lg:col-span-3">
              {feedType === 'personal' ? (
                  <UsersGrid 
                    users={users} // You'll need to fetch users data
                    title="My Network"
                    onUserClick={(user) => {handleUserClick}}
                />
              ) : (
